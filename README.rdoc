...
To do "string interpolation" after a string has already been 
...
String#interpolate will only interpolate for local methods.  It will not 
work to call a method or to evaluate other ruby code inside the #{pattern}.
Example:
def m; "HI"; end
'will not run the #{m} method'.interpolate() {}  #does nothing!
'will not add 1 + 1 =#{1 + 1}'.interpolate() {}  #does nothing!
...
For String#interpolate('') {},
  ...
  a_local_var = "I am the value of a_local_var"
  test_string = 'we have >>>#{a_local_var}<<< but we do not have >>>#{not_local_var}<<<'

  result      = test_string.interpolate('') {}
  result # => 'we have >>>I am the value of a_local_var<<< but we do not have >>><<<'
...
But, String#interpolate has many other options.  Here is an important one:
   ...
   result # => 'we have >>>I am the value of a_local_var<<< but we do not have >>>#{not_local_var}<<<'
...
useful(IMO) is the "String#interpolate() {}"
...
== Methods (We DONT support method interpolation, BUT...)
